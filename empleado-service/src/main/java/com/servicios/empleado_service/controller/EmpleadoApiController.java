...
public class EmpleadoApiController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<?> getEmpleadoById(@PathVariable Integer id, HttpServletRequest request) {
        Integer tokenId = (Integer) request.getAttribute("empleadoId");
        if (tokenId == null) return ResponseEntity.status(401).body("No autenticado");
        if (!tokenId.equals(id)) return ResponseEntity.status(403).body("No autorizado");
    ...
    @GetMapping("/me")
    public ResponseEntity<?> getEmpleadoMe(HttpServletRequest request) {
        Integer id = (Integer) request.getAttribute("empleadoId");
        if (id == null) return ResponseEntity.status(401).body("No autenticado");
        return getEmpleadoById(id, request);
    }
    ...
    @PutMapping("/me")
    public ResponseEntity<?> updateEmpleadoMe(@RequestBody Empleado payload, HttpServletRequest request) {
        Integer id = (Integer) request.getAttribute("empleadoId");
        if (id == null) return ResponseEntity.status(401).body("No autenticado");
        return updateEmpleadoById(id, payload, request);
    }
}
